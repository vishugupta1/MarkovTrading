...
def stch_indicator(input_df_close = pd.DataFrame(), input_df_low = pd.DataFrame(), input_df_high = pd.DataFrame()):
    ...
    return percent_k.append(percent_d)
...
def momentum_indicator(input_df = pd.DataFrame()):
    return input_df.pct_change(10).append(input_df.pct_change(14))
