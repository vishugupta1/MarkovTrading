...
def perform_analysis(symbol="IBM", sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009, 1, 1), sv=100000):
    ...
    portfolio_dataframe = pd.DataFrame(index=dates, columns=[symbol+"_BM",symbol+"_SL"])
    ...
    for date in stocks_data.index:
        benchmark_current_price = stocks_data.loc[date, symbol]
    ...
    for date in stocks_data.index:
        ...
        current_price = stocks_data.loc[date, symbol]
        current_trade_amount = current_trade_amount + current_trade
        if current_trade == 1000:
            long_amount = 1000
            cash_value = cash_value - (current_price*1000)
...
if __name__ == "__main__":
    perform_analysis(symbol="JPM", sd=dt.datetime(2008, 1, 1), ed=dt.datetime(2009, 1, 1), sv=100000) # insample
    perform_analysis(symbol="JPM", sd=dt.datetime(2010, 1, 1), ed=dt.datetime(2011, 1, 1), sv=100000) # outsample
